...
class Encoder(nn.Module):
    def __init__(self, args, num_users, num_items):
        ...
        if args.dataset in ['yelp1', 'flickr', 'douban-book']:
            self.user_emb_ego = torch.nn.Embedding(num_embeddings=num_users, embedding_dim=args.hidden_dim)
            self.item_emb_ego = torch.nn.Embedding(num_embeddings=num_items, embedding_dim=args.hidden_dim)
        ...
        self.prop_ui = Propagation(args.n_layers, args.dropout_adj) # U-I 图卷积
        self.prop_uu = Propagation(args.n_layers, args.dropout_adj) # U-U 图卷积
        self.num_users = num_users
        self.num_items = num_items

    def forward(self, ui_graph, uu_graph, user_feat, item_feat, train=True):
