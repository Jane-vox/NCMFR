...
class GSL4uu(nn.Module):
    ...
    def new_stru(self, user_emb, prob_dele_edge, prob_add_edge, all_nodes, cluster_nodes, batch_subgraph):
        ...
        dele_indices = batch_subgraph._indices() # 稀疏图的边索引
        dele_final_sim = []
        dele_final_indices = []
        for one_num_nei in num_nei:
            ...
            keep_edge_num = int(one_num_nei*(1-prob_i))  # 取整方式：ceil（向上取整），floor（向下取整），round（四舍五入）
        ...
        return dele_final_indices, dele_final_sim
